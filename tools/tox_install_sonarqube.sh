...
echo "sonar.host.url=${SONAR_SERVER_URL}" >> "${WORKDIR}/sonarqube/sonar-scanner-${SONAR_SCANNER_VER}-linux/conf/sonar-scanner.properties"

# Add trust certs for redhat internal websites to keystore
if [[ $SONAR_SERVER_URL =~ "redhat" ]]; then
