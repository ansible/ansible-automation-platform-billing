...
def fetchSubscriptionAndRG():
    ...
    j = _getJsonPayload(url, metadata_header, 'subscription and resource group')
    subId=j['compute']['subscriptionId']
    rg=j['compute']['resourceGroupName']
    logger.debug("Fetched subscription ID (%s) and resource group (%s)." % (subId, rg))
    return (subId, rg)
...
def fetchResourceIdAndPlan(managed_app_id, access_token):
    ...
    """
    auth_header = {"Authorization": "Bearer %s" % access_token}
    url = "https://management.azure.com%s?api-version=%s" % (managed_app_id, managed_app_api_version)
