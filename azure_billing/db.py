...
def getUnbilledHosts():
    ...
    executed_hosts = {x.host_name: x.modified for x in JobHostSummary.objects.order_by('host_name', '-modified').distinct('host_name')}
    billed_hosts = {x.host_name: x.billed_date for x in BilledHost.objects.all()}
    new_hosts = []
...
def recordBillingInstance(billing_data):
    ...
    """
    b = BillingRecord(hosts=",".join(billing_data['hosts']),
